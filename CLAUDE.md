...
- `winget` and Git checkpoints currently perform discovery/diagnosis/decision output only and must not install, repair, modify PATH, edit registry, or write user configuration; their non-healthy decisions are dependency blockers and return exit code `60`. The `winget` checkpoint may report Appx deployment repair facts and use them to fail closed when Appx deployment is unavailable, but must not download App Installer dependencies or call `Add-AppxPackage` in this stage.
- The App Installer download checkpoint is download-only staging: it defaults to planned/no-download, reports incomplete metadata explicitly, refuses `-AllowDownload` without URL/allowed-host/SHA-256 metadata, confines downloads/results to the user-local installer root, bounds retry/timeout inputs, cleans or invalidates hash-mismatched partial files, and must not execute, install, unpack, edit PATH, edit registry, or write user configuration.
